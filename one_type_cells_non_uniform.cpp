...
//    double constant = 29.12;
    ...
    // adapted, most of the simulations done with this
//    double L_0 = 30; // will have to make this consistent with actual initial length
...
//    double constant = 29.12;
    ...
    //for correct initial domain length
    double L_0 = 30; // will have to make this consistent with actual initial length
    //double a = 0.001;//0.008;//0.23/10;
    double a = 0.23/20;
    double L_inf = 86.76;
...
//        }
        ...
        for (int i = 0; i < length_x; i++) {
            for (int j = 0; j < length_y; j++) {
                ...
                for (int k = 0; k < particles.size(); k++) {
                    ...
                    // scaling_factor = (domain_length )/(length_x );//uniform growth in the first part of the domain
                    // intern(i,j) = intern(i,j) + exp(- ((scaling_factor*i-x[0])*(scaling_factor*i-x[0])+(j-x[1])*(j-x[1]))/(2*R*R)); // mapping to fixed domain
        ...
        // equation for uniform growth
//        for (int i = 1; i < length_x - 1; i++) {
//            for (int j = 1; j < length_y - 1; j++) {
//
//                // logistic production rate
//                chemo_new(i, j) = dt * (D * ((1 / ((domain_length / double(length_x)) *
//                                                   (domain_length / double(length_x)))) *
//                                             (chemo(i + 1, j) - 2 * chemo(i, j) + chemo(i - 1, j)) / (dx * dx) +
//                                             (chemo(i, j + 1) - 2 * chemo(i, j) + chemo(i, j - 1)) / (dy * dy)) -
//                                        (chemo(i, j) * lam / (2 * M_PI * R * R)) * intern(i, j) +
//                                        kai * chemo(i, j) * (1 - chemo(i, j)) -
...
//        }
        ...
        /// update positions uniformly based on the domain growth, uniform
